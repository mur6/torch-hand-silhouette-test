...
class FreiHandDataset(Dataset):
    def __init__(
        ...
        # print(os.path.join(path, joints_anno_file))
...
class FreiHandDataset_Estimated(Dataset):
    def __init__(
        ...
        with open(os.path.join(path, vertices_anno_file), "r") as fh:
            self.vertices = json.load(fh)
            self.vertices = (np.array(self.vertices)[split_ids]).tolist()

    def __getitem__(self, index):
        image_name = self.image_paths[index]
        image = cv2.imread(image_name, cv2.IMREAD_GRAYSCALE)  # grayscale image 224 x 224
        orig_image = image
        print(f"Camera Ks: {self.camera_Ks[index]}")
        ...
        return orig_image, image, focal_len, image_ref, label, dist_map, mesh
